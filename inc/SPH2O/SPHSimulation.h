...
#include "FluidObject/FluidObject.h"
#include "BoundaryObject/BoundaryObject.h"
#include "NonPressureForce/NonPressureForce.h"
#include "TimeHelper.h"
#include "SPHSolver/SPHSolver.h"
#include "HashTable.h"
...
class SPHSimulation : public SimulationSystem
{
    ...
    public:
        ...
        bool step() override; // TODO: dejar step en void y crear algun método para hacer if (nohaacabado) step()
        ...
        // Setters
        void setParticleRadius(const Real);
        void setSupportRadius(const Real);
        void setGravity(const Vector3r& gravity) { this->gravity = gravity; }
        ...
        void activateSave(const bool active) { activeSave = active; }
        // TODO: gestionar mejor los metodos relacionados con el tiempo
        void setTime(const Real time) { tm.setTime(time); }
        void setStartTime(const Real startTime) { tm.setStartTime(startTime); }
        void setEndTime(const Real endTime) { tm.setEndTime(endTime); }
        void setTimeStep(const Real ts) { tm.setTimeStep(ts); }
        void setFPS(const Real fps) { tm.setFPS(fps); }
        void setMinTimeStep(const Real minTs) { tm.setMinTimeStep(minTs); }
        void setMaxTimeStep(const Real maxTs) { tm.setMaxTimeStep(maxTs); }
        ...
        void startCounting(const std::string& name) { tm.startCounting(name); }
        void stopCounting(const std::string& name) { tm.stopCounting(name); }
        double getInterval(const std::string& name) { return tm.getInterval(name); }
        ...
        SimulationObject* getSimObject(const uint i) override 
        {
        ...
        void addBoundaryModel(BoundaryObject* bObj);
        ...
        bool importFluidState(const uint frame);
        void clearSimInfo();
